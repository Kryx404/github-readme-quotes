...
getQuoteIndex = (apiResponseLength, quoteType) => {
    ...
    return quoteType === "quote-for-the-day"
        ? epoch % apiResponseLength
        : Math.random() * apiResponseLength;
};
    ...
    try {
        ...
        if (isValidUrl) {
            ...
            if (apiResponse.length > 0) {
                apiResponse =
                    apiResponse[
                        Math.floor(getQuoteIndex(apiResponse.length, quoteType))
                    ];
                if (!apiResponse.quote && !apiResponse.author) {
                    apiResponse =
                        defaultQuotes[
                            Math.floor(
                                getQuoteIndex(defaultQuotes.length, quoteType),
                            )
                        ];
                } else {
                    isCustomQuote = true;
            ...
            apiResponse =
                defaultQuotes[
                    Math.floor(getQuoteIndex(defaultQuotes.length, quoteType))
                ];
            isCustomQuote = true;
        }
